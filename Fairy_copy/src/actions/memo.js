...
export function memoListRequest(id,username) {
    return (dispatch) => {
        console.log("memolistREquesttest");
        ...
        if (typeof username === "undefined") {
            ...
            // /api/memo/old/meme2367
            url = username ? url : `${url}/${id}/`
        } else if (typeof id === "undefined"){
            //load memos of specific user
            url = id ? `${url}` : `${url}/${username}`;
        }else {
            url = `${url}/${username}/${id}`;
        }
...
export function memoListSuccess(data) {
    return {
